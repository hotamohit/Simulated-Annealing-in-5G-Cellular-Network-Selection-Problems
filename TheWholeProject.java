...
public class TheWholeProject extends Frame{

	private static final long serialVersionUID = 1L;
	private static int stations = 35;
	private static int clients = 100;
	private static double NA_SW=0;//social welfare of nash equilibrium
	...
	public static void main(String[] args) {
		...
		TheWholeProject sa = new TheWholeProject();
...
//		System.out.println(weightList);
	...
	public void balancing() {
		...
		TheWholeProject ld = new TheWholeProject();
		int sum;
...
//					System.out.println("\n**********New Allocation**********");
	...
	public void annealing() {
		...
		double OF1 = 0, OF2 = 0; //objective function
		double T = 3000, k0 = 0.00001; //T is temperature, k0 is Boltzmann's constant
		double probability;
		...
		while (T > 0.1) {
			...
			for (int i=0; i<clients; i++) {
				OF1 += Math.log10(1 + getSINR(i));
			}
			
			change();
			
			for (int i=0; i<clients; i++) {
				OF2 += Math.log10(1 + getSINR(i));
			}
			
			/*System.out.println("OF1 = " + OF1 + ", OF2 = " + OF2);*/
			
			if (OF2 > OF1) { //if the change increases objective function, accept
...
//				System.out.println();
				...
				probability = Math.pow(Math.E, (OF2-OF1)/(k0*T));
//				System.out.println("probability = " + probability);
				
				if (probability < 0.0000001) { //if the probability becomes too small, end the algorithm
					break;
				}
