...
public class SimulatedAnnealingEx extends Frame{
	...
	public static void main(String[] args) {
		...
		SimulatedAnnealingEx sa = new SimulatedAnnealingEx();
	...
	public void annealing() {
		...
		double sumOfThroughput1 = 0, sumOfThroughput2 = 0; //objective function
		...
		while (T > 1) {
			...
			for (int i=0; i<clients; i++) {
				sumOfThroughput1 += Math.log10(1 + getSINR(i));
			}
			
			change();
			
			for (int i=0; i<clients; i++) {
				sumOfThroughput2 += Math.log10(1 + getSINR(i));
			}
		...
		if (accept == 0) { //not accepted
			resultOfSA = sumOfThroughput1;
			System.out.println("Sum of throughput is " + sumOfThroughput1 + " from simulated annealing");
		}
		else { //accepted
			resultOfSA = sumOfThroughput2;
			System.out.println("Sum of throughput is " + sumOfThroughput2 + " from simulated annealing");
		}
	...
	public void enumerate() {
		...
		for (int i=0; i<Math.pow(stations, clients); i++) {
			...
			for (int j=0; j<clients; j++) {
				sumOfThroughput += Math.log10(1 + getSINR(j, using));
				result.add(sumOfThroughput);
			}
		...
		System.out.println("Sum of throughput is " + bestResult + " from enumeration");
	}
